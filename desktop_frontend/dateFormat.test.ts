...
describe("formatDateShort", () => {
  it("formats YYYY-MM-DD as DD.MM.YYYY", () => {
    expect(formatDateShort("2026-08-01")).toBe("01.08.2026");
  });

  it("formats ISO datetime string", () => {
    expect(formatDateShort("2026-12-25T14:30:00")).toBe("25.12.2026");
  });
...
describe("formatDateLong", () => {
  it("includes long weekday", () => {
    ...
    // Should be like "Saturday, 01.08.2026"
    expect(result).toMatch(
      /^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday), \d{2}\.\d{2}\.\d{4}$/,
    );
  });
});
...
describe("formatDateTime", () => {
  it("formats datetime as DD.MM.YYYY HH:MM", () => {
    ...
    expect(result).toBe("01.08.2026 14:30");
  });
