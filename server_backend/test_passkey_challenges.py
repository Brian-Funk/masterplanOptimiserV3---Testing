...
def test_bootstrap_begin_requires_discoverable_credentials(db):
    """Root bootstrap passkey registration requires discoverable credentials."""
    create_test_user(
        db,
        username="discoverable.bootstrap",
        is_root_admin=True,
        is_admin=True,
    )
    client = _raw_client()

    response = client.post("/api/v1/passkey/bootstrap/begin")
